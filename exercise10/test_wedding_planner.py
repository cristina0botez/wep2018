from wedding_planner import GuestList, Person, TableFull
import pytest
...
def populated_tables(some_people):
    gl = GuestList(12, 10)
    for table_number, one_person in enumerate(some_people, start=1):
...
def test_with_a_few_people(some_people):
    gl = GuestList(10, 10)
    for table_number, one_person in enumerate(some_people[:5], start=1):
        if table_number == 0:
...
def test_repr(some_people):
    ...
    for one_person in assigned_people:
        assert '{0.last}, {0.first}'.format(one_person) in output
